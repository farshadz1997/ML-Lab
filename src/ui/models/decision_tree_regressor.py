...
Tree-based regressor that recursively partitions data based on feature values
to minimize variance at each node for continuous value prediction.
...
class DecisionTreeRegressorModel:
    """Decision Tree regressor with configurable hyperparameters."""
    ...
    def _prepare_data(self) -> Optional[Tuple]:
        """
        Preprocess, encode, scale (on full X), and split data for regression.
        ...
        try:
            ...
            if y.dtype == 'object':
                self.parent.page.open(ft.SnackBar(
                    ft.Text("Target column must be numeric for regression", font_family="SF regular")
                ))
                return None
            else:
                y = y.values
            ...
            content=ft.Container(
                ...
                content=ft.Column(
                    ...
                    controls=[
                        ft.Row(
                            controls=[
                                ft.Text("Decision Tree Regressor",
                                       font_family="SF thin",
                                       size=24,
                                       text_align="center",
                                       expand=True)
